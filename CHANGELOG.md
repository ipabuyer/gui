...
1. 新增功能：同步账户已拥有 App 列表（基于 ipatool 2.5.0 新增的 `list-purchases` 命令）
   1. 触发时机：登录后从未同步过、启动时距上次成功同步超过 7 天（后台静默）、设置页“刷新已购买列表”手动触发（含进度显示）
   2. 同步状态按账户存于数据库 `SyncState` 表，自动同步按上次成功时间判定阈值
2. 合并“已拥有”状态到“已购买”：
   1. 数据库迁移归并旧的“已拥有”记录
   2. 主页筛选与卡片菜单移除“已拥有”
   3. 购买返回 STDQ 报错或 alreadyOwned 时直接标记为已购买，移除确认弹窗与 `OwnedCheckEnabled` 设置
3. 内置 ipatool 版本：2.4.0 -> 2.5.0；ipatool 页新增版本要求卡片（要求 ≥ 2.5.0，2.5.0 含破坏性更改，旧版本可能导致购买、下载失败）
4. 修复语言切换重启弹窗显示资源键名而非文案的问题
